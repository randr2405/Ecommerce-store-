...
function ClickSpark({ children, sparkColor = '#5227FF', sparkSize = 8, sparkRadius = 14, sparkCount = 8, duration = 400 }) {
    ...
    <Link href={`/shop?category=${cat.label.toLowerCase()}`} style={{ textDecoration: 'none', display: 'block' }}>
      ...
      >
        <BorderGlow
          ...
          coneSpread={25}
          animated={false}
          colors={['#C9A84C', '#EDD070', '#8B6914']}
        >
          <div style={{
            padding: '3.5rem 2.4rem',
            position: 'relative', overflow: 'hidden',
            transformStyle: 'preserve-3d',
            transform: `rotateY(${tiltY}deg) rotateX(${tiltX}deg) ${hovered ? 'translateZ(16px)' : ''}`,
            transition: hovered
...
export default function HomePage() {
  ...
  return (
    <ClickSpark sparkColor="#5227FF" sparkSize={7} sparkRadius={14} sparkCount={8} duration={400}>
      <div style={{ paddingTop: '70px', background: '#040302', overflowX: 'hidden' }}>
        ...
        <section ref={heroRef} style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '4rem 2rem', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: '800px', height: '800px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(82,39,255,0.06) 0%, rgba(82,39,255,0.015) 40%, transparent 65%)', pointerEvents: 'none', zIndex: 1, animation: 'rrBloom 4s ease-in-out infinite alternate' }} />
        ...
        <section ref={brandRef} style={{ position: 'relative', zIndex: 2, overflow: 'hidden', perspective: '1200px' }}>
          <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
            <LiquidChrome
          ...
          <div style={{ borderTop: '1px solid rgba(201,168,76,0.1)', borderBottom: '1px solid rgba(201,168,76,0.1)', padding: '11rem 2rem', textAlign: 'center', position: 'relative', zIndex: 2 }}>
